...
const FollowerScreen = ({ route }) => {
    ...
    const fetchFollowerList = async () => {
        try {
            const response = await fetch(`${Server}/user/get-list/follower?currentUser=${currentUser}&targetUser=${targetUser}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            });

            if (!response.ok) {
                throw new Error('팔로워 목록을 불러오는 중 오류가 발생했습니다.');
            }
    ...
    return (
        <SafeAreaView style={CONTAINER.container}>
            <View style={[CONTAINER.header, { justifyContent: 'center', alignItems: 'center', height: '5%', marginBottom: 5 }]} >
                <Text style={TYPOGRAPHY.bigText}>팔로워</Text>
            </View>
